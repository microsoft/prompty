...
class PromptyBase(ABC):
    ...
    @staticmethod
    def load(data: Any) -> "PromptyBase":
        """Load a PromptyBase instance."""

        if not isinstance(data, dict):
            raise ValueError(f"Invalid data for PromptyBase: {data}")

        # load polymorphic PromptyBase instance
        instance = PromptyBase.load_kind(data)
        ...
        if data is not None and "inputs" in data:
            instance.inputs = PromptyBase.load_inputs(data["inputs"])
        if data is not None and "outputs" in data:
            instance.outputs = PromptyBase.load_outputs(data["outputs"])
        if data is not None and "tools" in data:
            instance.tools = PromptyBase.load_tools(data["tools"])
        return instance
    ...
    @staticmethod
    def load_kind(data: dict) -> "PromptyBase":
        # load polymorphic PromptyBase instance
        if data is not None and "kind" in data:
            ...
            raise ValueError("Missing PromptyBase discriminator property: 'kind'")


@dataclass
class Prompty(PromptyBase):
...
@dataclass
class PromptyContainer(PromptyBase):
    """The following represents a containerized agent that can be deployed and hosted.It includes details about the container image, registry information, and environment variables.This model allows for the definition of agents that can run in isolated environments,making them suitable for deployment in various cloud or on-premises scenarios.The containerized agent can communicate using specified protocols and can be scaledbased on the provided configuration.This kind of agent represents the users intent to bring their own container specificapp hosting platform that they manage.
    Attributes
    ----------
    kind : str
...
@dataclass
class PromptyHostedContainer(PromptyBase):
    ...
    protocol: str = field(default="responses")
    container: HostedContainerDefinition = field(default_factory=HostedContainerDefinition)
    environmentVariables: Optional[list[EnvironmentVariable]] = field(default_factory=list)

    @staticmethod
    def load(data: Any) -> "PromptyHostedContainer":
        """Load a PromptyHostedContainer instance."""

        if not isinstance(data, dict):
            raise ValueError(f"Invalid data for PromptyHostedContainer: {data}")

        # create new instance
        instance = PromptyHostedContainer()
        if data is not None and "kind" in data:
            instance.kind = data["kind"]
        if data is not None and "protocol" in data:
            instance.protocol = data["protocol"]
        if data is not None and "container" in data:
            instance.container = HostedContainerDefinition.load(data["container"])
        if data is not None and "environmentVariables" in data:
            instance.environmentVariables = PromptyHostedContainer.load_environmentVariables(
