...
public struct EngineCheckpoint: TypraModel {
    ...
    -> EngineCheckpoint
  {
    ...
    if let value = object["pendingToolRequests"] {
      instance.pendingToolRequests = try TypraRuntime.array(value, field: "pendingToolRequests").map
    ...
    if let value = object["completedToolResults"] {
      instance.completedToolResults = try TypraRuntime.array(value, field: "completedToolResults")
        .map { try ModelToolResult.load($0, context: context) }
    }
    ...
    if let value = object["pendingModelResponse"] {
      instance.pendingModelResponse = try ModelInvocationResponse.load(value, context: context)
    }
