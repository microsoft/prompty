...
public struct ModelReconciliationState: TypraModel {
  ...
  public static func load(_ data: Any, context: LoadContext = LoadContext()) throws -> ModelReconciliationState {
    ...
    if object["request"] == nil || object["request"] is NSNull {
      throw TypraRuntimeError.unsupported(context.at("request").path + ": missing required field")
    }
    if let value = object["request"] {
      instance.request = try ModelInvocationRequest.load(value, context: context.at("request"))
    }
