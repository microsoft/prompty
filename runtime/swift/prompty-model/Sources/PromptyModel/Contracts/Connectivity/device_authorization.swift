...
public struct DeviceAuthorization: TypraModel {
  ...
  public func toWire(_ provider: String, context: SaveContext = SaveContext()) throws -> [String: Any] {
    ...
    default: wireNameDeviceCode = nil
    }
    if let wireKey = wireNameDeviceCode { result[wireKey] = self.deviceCode }
    let wireNameUserCode: String?
    switch provider {
    case "foundry": wireNameUserCode = "user_code"
    default: wireNameUserCode = nil
    }
    if let wireKey = wireNameUserCode { result[wireKey] = self.userCode }
    let wireNameVerificationUri: String?
    switch provider {
    case "foundry": wireNameVerificationUri = "verification_uri"
    default: wireNameVerificationUri = nil
    }
    if let wireKey = wireNameVerificationUri { result[wireKey] = self.verificationUri }
    ...
    default: wireNameInterval = nil
    }
    if let wireKey = wireNameInterval { result[wireKey] = self.interval }
    let wireNameMessage: String?
    switch provider {
    case "foundry": wireNameMessage = "message"
    default: wireNameMessage = nil
    }
    if let wireKey = wireNameMessage { result[wireKey] = self.message }
    return result
  }

  public static func fromWire(_ provider: String, _ data: [String: Any], context: LoadContext = LoadContext()) throws -> DeviceAuthorization {
