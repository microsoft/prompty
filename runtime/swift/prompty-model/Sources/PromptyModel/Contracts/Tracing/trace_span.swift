...
public struct TraceSpan: TypraModel {
  ...
  public static func load(_ data: Any, context: LoadContext = LoadContext()) throws -> TraceSpan {
    ...
    if object["__time"] == nil || object["__time"] is NSNull {
      throw TypraRuntimeError.unsupported(context.at("__time").path + ": missing required field")
    }
    if let value = object["__time"] {
      instance.time = try TraceTime.load(value, context: context.at("__time"))
    }
    ...
    if let value = object["__usage"] {
      instance.usage = try TokenUsage.load(value, context: context.at("__usage"))
    }
