...
/**
 * Grades the Anthropic half of the shared discovery and enrichment suites.
 *
...
class DiscoveryVectorsTest {
  ...
  Iterable<DynamicTest> discoveryVectors() {
    ...
    for (Map<String, Object> vector : SpecVectors.readCases("discovery/discovery_vectors.json", "vectors")) {
      if (!"anthropic".equals(vector.get("provider"))) {
        continue;
      }
      ...
      tests.add(
          DynamicTest.dynamicTest(
              ...
              () -> {
                ModelInfo actual =
                    AnthropicModelLister.modelInfoFromWire(SpecVectors.map(vector, "input"));
      ...
      tests.add(
          DynamicTest.dynamicTest(
              ...
              () -> {
                ...
                Discovery.enrich("anthropic", info);
                SpecVectors.assertEquivalent(
