...
public final class Guardrails {
  ...
  public interface Input {
    GuardrailResult check(List<Message> messages, Prompty agent);
  }

  /** Checked against the final output, once no more tool calls are outstanding. */
  @FunctionalInterface
  public interface Output {
    GuardrailResult check(Object output, Prompty agent);
  }

  /** Checked before each tool execution, against the tool's name and parsed arguments. */
  @FunctionalInterface
  public interface Tool {
    GuardrailResult check(String name, Object arguments, Prompty agent);
  }
  ...
  public GuardrailResult checkInput(List<Message> messages, Prompty agent) {
    return input == null ? GuardrailResult.allow() : input.check(messages, agent);
  }

  /** Run the output hook, or allow if none is configured. */
  public GuardrailResult checkOutput(Object output, Prompty agent) {
  ...
  public GuardrailResult checkTool(String name, Object arguments, Prompty agent) {
