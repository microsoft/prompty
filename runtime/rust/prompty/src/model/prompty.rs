...
impl Prompty {
    ...
    pub fn load_from_value(value: &serde_json::Value) -> Self {
        Self {
            ...
            model: value.get("model").filter(|v| v.is_object() || v.is_array() || v.is_string()).map(Model::load_from_value).unwrap_or_default(),
            tools: value.get("tools").cloned().unwrap_or(serde_json::Value::Null),
            template: value.get("template").filter(|v| v.is_object() || v.is_array() || v.is_string()).map(Template::load_from_value),
