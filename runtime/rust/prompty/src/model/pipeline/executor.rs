...
pub trait Executor: Send + Sync {
    ...
    async fn execute(&self, agent: &Prompty, messages: &Vec<Message>, cancellation: &CancellationToken) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>>;
    ...
    async fn execute_stream(&self, agent: &Prompty, messages: &Vec<Message>, cancellation: &CancellationToken) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
