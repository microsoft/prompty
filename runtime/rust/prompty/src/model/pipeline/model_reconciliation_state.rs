...
impl ModelReconciliationState {
    ...
    pub fn load_from_value(value: &serde_json::Value, ctx: &LoadContext) -> Self {
        ...
        Self {
            ...
            request: value.get("request").filter(|v| v.is_object() || v.is_array() || v.is_string()).map(|v| ModelInvocationRequest::load_from_value(v, ctx)).unwrap_or_default(),
            failed_attempt: value.get("failedAttempt").and_then(|v| v.as_i64()).unwrap_or(0) as i32,
    ...
    pub(crate) fn validate_input_at(value: &serde_json::Value, path: &str) -> Result<(), String> {
        let child_path = if path.is_empty() { "request".to_string() } else { format!("{}.request", path) };
        let child = value.get("request").filter(|candidate| !candidate.is_null())
    ...
    pub fn to_value(&self, ctx: &SaveContext) -> serde_json::Value {
        ...
        if self.failed_attempt != 0 {
            result.insert("failedAttempt".to_string(), serde_json::Value::Number(serde_json::Number::from(self.failed_attempt)));
        }
...
// Serde for `ModelReconciliationState` delegates to the canonical to_value/load_from_value
...
impl serde::Serialize for ModelReconciliationState {
