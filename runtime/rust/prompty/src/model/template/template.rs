...
impl Template {
    ...
    pub fn load_from_value(value: &serde_json::Value, ctx: &LoadContext) -> Self {
        ...
        Self {
            format: value.get("format").filter(|v| v.is_object() || v.is_array() || v.is_string()).map(|v| FormatConfig::load_from_value(v, ctx)).unwrap_or_default(),
            parser: value.get("parser").filter(|v| v.is_object() || v.is_array() || v.is_string()).map(|v| ParserConfig::load_from_value(v, ctx)).unwrap_or_default(),
        }
    }

    pub(crate) fn validate_input_at(value: &serde_json::Value, path: &str) -> Result<(), String> {
        let child_path = if path.is_empty() { "format".to_string() } else { format!("{}.format", path) };
        let child = value.get("format").filter(|candidate| !candidate.is_null())
            .ok_or_else(|| format!("{}: missing required field", child_path))?;
        FormatConfig::validate_input_at(child, &child_path)?;
        let child_path = if path.is_empty() { "parser".to_string() } else { format!("{}.parser", path) };
        let child = value.get("parser").filter(|candidate| !candidate.is_null())
            .ok_or_else(|| format!("{}: missing required field", child_path))?;
