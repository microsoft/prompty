...
impl Tool {
    ...
    pub(crate) fn validate_input_at(value: &serde_json::Value, path: &str) -> Result<(), String> {
        if let Some(collection) = value.get("bindings") {
            let collection_path = if path.is_empty() { "bindings".to_string() } else { format!("{}.bindings", path) };
            match collection {
        ...
        let discriminator = match discriminator {
            ...
            _ => return Err("Invalid Tool discriminator field 'kind': expected non-blank string".to_string()),
        };
        match discriminator {
            "function" => {
                if let Some(collection) = value.get("parameters") {
                    let collection_path = if path.is_empty() { "parameters".to_string() } else { format!("{}.parameters", path) };
                    match collection {
                        serde_json::Value::Object(entries) => {
                            for (name, entry) in entries {
                                let entry_path = format!("{}.{}", collection_path, name);
            ...
            "mcp" => {
                ...
                let child = value.get("connection").filter(|candidate| !candidate.is_null())
    ...
    pub fn to_value(&self, ctx: &SaveContext) -> serde_json::Value {
        ...
        match &self.kind {
            ToolKind::Function { parameters, strict,  .. } => {
                result.insert("parameters".to_string(), Self::save_parameters(parameters, ctx));
            ...
            ToolKind::Custom { connection, options, raw, .. } => {
                result.insert("connection".to_string(), connection.clone());
                result.insert("options".to_string(), options.clone());
                for (key, value) in raw {
                    if matches!(key.as_str(), "name" | "kind" | "description" | "bindings" | "connection" | "options") { continue; }
        ...
        match data {
            ...
            serde_json::Value::Object(obj) => {
                obj.iter()
                    .map(|(name, value)| {
                        if value.is_array() {
                            panic!("bindings.{}: invalid named collection entry category array", name);
                        }
        ...
        match data {
            ...
            serde_json::Value::Object(obj) => {
                obj.iter()
                    .map(|(name, value)| {
                        if value.is_array() {
                            panic!("parameters.{}: invalid named collection entry category array", name);
                        }
                        let mut v = if value.is_object() {
                            value.clone()
                        } else {
...
impl serde::Serialize for ToolKind {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let parent = Tool { kind: self.clone(), ..Default::default() };
