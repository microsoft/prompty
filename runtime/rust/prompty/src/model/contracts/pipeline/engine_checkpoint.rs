...
impl EngineCheckpoint {
    ...
    /// Load EngineCheckpoint from an already-parsed JSON value, returning an error
    ...
    pub fn to_value(&self, ctx: &SaveContext) -> serde_json::Value {
        ...
        if let Some(items) = self.pending_tool_requests.as_ref() {
            result.insert(
                "pendingToolRequests".to_string(),
                Self::save_pending_tool_requests(items, ctx),
            );
        }
        if let Some(items) = self.completed_tool_results.as_ref() {
            result.insert(
                "completedToolResults".to_string(),
                Self::save_completed_tool_results(items, ctx),
            );
        }
        result.insert(
            "completedModelIterations".to_string(),
            serde_json::Value::Number(serde_json::Number::from(self.completed_model_iterations)),
        );
        result.insert(
