...
impl ModelInvocationResponse {
    ...
    /// Load ModelInvocationResponse from an already-parsed JSON value, returning an error
    ...
    pub fn to_value(&self, ctx: &SaveContext) -> serde_json::Value {
        ...
        if let Some(items) = self.assistant_messages.as_ref() {
            result.insert(
                "assistantMessages".to_string(),
                Self::save_assistant_messages(items, ctx),
            );
        }
        if let Some(items) = self.tool_requests.as_ref() {
            result.insert(
                "toolRequests".to_string(),
                Self::save_tool_requests(items, ctx),
            );
        }
