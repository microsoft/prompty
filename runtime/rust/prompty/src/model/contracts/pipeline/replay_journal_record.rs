...
impl<'de> serde::Deserialize<'de> for ReplayRecordKind {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str_opt(&s)
            .ok_or_else(|| serde::de::Error::custom(format!("invalid ReplayRecordKind value: {}", s)))
    }
}
...
impl<'de> serde::Deserialize<'de> for ReplayRecordStatus {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let s = <String as serde::Deserialize>::deserialize(deserializer)?;
        Self::from_str_opt(&s)
            .ok_or_else(|| serde::de::Error::custom(format!("invalid ReplayRecordStatus value: {}", s)))
    }
}
...
impl ReplayJournalRecord {
    ...
    pub fn load_from_value(value: &serde_json::Value, ctx: &LoadContext) -> Self {
        ...
        Self {
            kind: value.get("kind").and_then(|v| v.as_str()).and_then(|s| ReplayRecordKind::from_str_opt(s)).unwrap_or(ReplayRecordKind::Session),
            ...
            status: value.get("status").and_then(|v| v.as_str()).and_then(|s| ReplayRecordStatus::from_str_opt(s)),
            request_id: value.get("requestId").and_then(|v| v.as_str()).map(|s| s.to_string()),
            tool_name: value.get("toolName").and_then(|v| v.as_str()).map(|s| s.to_string()),
    ...
    pub fn to_value(&self, ctx: &SaveContext) -> serde_json::Value {
        ...
        if let Some(val) = self.tool_name.as_ref() {
            result.insert("toolName".to_string(), serde_json::Value::String(val.clone()));
        }
