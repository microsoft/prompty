...
impl<'de> serde::Deserialize<'de> for InvocationContextDisposition {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str_opt(&s)
            .ok_or_else(|| serde::de::Error::custom(format!("invalid InvocationContextDisposition value: {}", s)))
    }
}
...
impl InvocationContextDecision {
    ...
    pub fn load_from_value(value: &serde_json::Value, ctx: &LoadContext) -> Self {
        ...
        Self {
            candidate_id: value.get("candidateId").and_then(|v| v.as_str()).unwrap_or_default().to_string(),
            disposition: value.get("disposition").and_then(|v| v.as_str()).and_then(|s| InvocationContextDisposition::from_str_opt(s)).unwrap_or(InvocationContextDisposition::Included),
            ...
            estimated_tokens: value.get("estimatedTokens").and_then(|v| v.as_i64()).map(|v| v as i32),
    ...
    pub fn to_value(&self, ctx: &SaveContext) -> serde_json::Value {
        ...
        // Write base fields
        result.insert("candidateId".to_string(), serde_json::Value::String(self.candidate_id.clone()));
        result.insert("disposition".to_string(), serde_json::Value::String(self.disposition.to_string()));
        result.insert("reason".to_string(), serde_json::Value::String(self.reason.clone()));
        if let Some(val) = self.rank.as_ref() {
            result.insert("rank".to_string(), serde_json::Value::Number(serde_json::Number::from(*val)));
        }
        if let Some(val) = self.estimated_tokens.as_ref() {
            result.insert("estimatedTokens".to_string(), serde_json::Value::Number(serde_json::Number::from(*val)));
        }
