...
impl TraceFile {
    ...
    pub fn load_from_value(value: &serde_json::Value, ctx: &LoadContext) -> Self {
        ...
        Self {
            ...
            trace: value.get("trace").filter(|v| v.is_object() || v.is_array() || v.is_string()).map(|v| TraceSpan::load_from_value(v, ctx)).unwrap_or_default(),
        }
    }

    pub(crate) fn validate_input_at(value: &serde_json::Value, path: &str) -> Result<(), String> {
        let child_path = if path.is_empty() { "trace".to_string() } else { format!("{}.trace", path) };
        let child = value.get("trace").filter(|candidate| !candidate.is_null())
