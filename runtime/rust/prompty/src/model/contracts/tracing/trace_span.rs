...
impl TraceSpan {
    ...
    pub(crate) fn validate_input_at(value: &serde_json::Value, path: &str) -> Result<(), String> {
        let child_path = if path.is_empty() { "__time".to_string() } else { format!("{}.__time", path) };
        let child = value.get("__time").filter(|candidate| !candidate.is_null())
            .ok_or_else(|| format!("{}: missing required field", child_path))?;
        TraceTime::validate_input_at(child, &child_path)?;
        let child_path = if path.is_empty() { "__usage".to_string() } else { format!("{}.__usage", path) };
