...
impl TurnEndPayload {
    ...
    pub fn load_from_value(value: &serde_json::Value, ctx: &LoadContext) -> Self {
        ...
        Self {
            iterations: value.get("iterations").and_then(|v| v.as_i64()).map(|v| v as i32),
            status: value.get("status").and_then(|v| v.as_str()).and_then(|s| TurnStatus::from_str_opt(s)),
    ...
    pub fn to_value(&self, ctx: &SaveContext) -> serde_json::Value {
        ...
        if let Some(val) = self.iterations.as_ref() {
            result.insert("iterations".to_string(), serde_json::Value::Number(serde_json::Number::from(*val)));
        }
