...
impl MessagesUpdatedPayload {
    ...
    pub fn load_from_value(value: &serde_json::Value, ctx: &LoadContext) -> Self {
        ...
        Self {
            ...
            removed: value.get("removed").and_then(|v| v.as_i64()).map(|v| v as i32),
        }
    }
    ...
    pub(crate) fn validate_input_at(value: &serde_json::Value, path: &str) -> Result<(), String> {
        ...
        if let Some(entries) = value.get("appended").and_then(|candidate| candidate.as_array()) {
            let collection_path = if path.is_empty() { "appended".to_string() } else { format!("{}.appended", path) };
            for (index, entry) in entries.iter().enumerate() {
                let entry_path = format!("{}[{}]", collection_path, index);
                Message::validate_input_at(entry, &entry_path)?;
            }
    ...
    pub fn to_value(&self, ctx: &SaveContext) -> serde_json::Value {
        ...
        if let Some(val) = self.removed.as_ref() {
            result.insert("removed".to_string(), serde_json::Value::Number(serde_json::Number::from(*val)));
        }
