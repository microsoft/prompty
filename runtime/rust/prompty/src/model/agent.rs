...
impl Agent {
    ...
    pub(crate) fn validate_input_at(value: &serde_json::Value, path: &str) -> Result<(), String> {
        if let Some(collection) = value.get("inputs") {
            let collection_path = if path.is_empty() { "inputs".to_string() } else { format!("{}.inputs", path) };
            match collection {
        ...
        if let Some(collection) = value.get("outputs") {
            let collection_path = if path.is_empty() { "outputs".to_string() } else { format!("{}.outputs", path) };
            match collection {
                serde_json::Value::Object(entries) => {
                    for (name, entry) in entries {
                        let entry_path = format!("{}.{}", collection_path, name);
        ...
        let child_path = if path.is_empty() { "template".to_string() } else { format!("{}.template", path) };
        ...
        match data {
            ...
            serde_json::Value::Object(obj) => {
                obj.iter()
                    .map(|(name, value)| {
                        if value.is_array() {
                            panic!("inputs.{}: invalid named collection entry category array", name);
                        }
        ...
        match data {
            ...
            serde_json::Value::Object(obj) => {
                obj.iter()
                    .map(|(name, value)| {
                        if value.is_array() {
                            panic!("outputs.{}: invalid named collection entry category array", name);
                        }
                        let mut v = if value.is_object() {
                            value.clone()
                        } else {
        ...
        match data {
            ...
            serde_json::Value::Object(obj) => {
                obj.iter()
                    .map(|(name, value)| {
                        if value.is_array() {
                            panic!("tools.{}: invalid named collection entry category array", name);
                        }
                        let mut v = if value.is_object() {
                            value.clone()
                        } else {
