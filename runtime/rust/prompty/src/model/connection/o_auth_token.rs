...
impl OAuthToken {
    ...
    pub fn load_from_value(value: &serde_json::Value, ctx: &LoadContext) -> Self {
        ...
        Self {
            access_token: value.get("accessToken").and_then(|v| v.as_str()).unwrap_or_default().to_string(),
            token_type: value.get("tokenType").and_then(|v| v.as_str()).unwrap_or_default().to_string(),
            expires_in: value.get("expiresIn").and_then(|v| v.as_i64()).unwrap_or(0),
            refresh_token: value.get("refreshToken").and_then(|v| v.as_str()).map(|s| s.to_string()),
    ...
    pub fn to_value(&self, ctx: &SaveContext) -> serde_json::Value {
        ...
        // Write base fields
        result.insert("accessToken".to_string(), serde_json::Value::String(self.access_token.clone()));
        result.insert("tokenType".to_string(), serde_json::Value::String(self.token_type.clone()));
        ...
        if let Some(val) = self.refresh_token.as_ref() {
            result.insert("refreshToken".to_string(), serde_json::Value::String(val.clone()));
        }
    ...
    pub fn to_wire(&self, provider: &str) -> serde_json::Value {
        ...
        let wire_map: std::collections::HashMap<&str, std::collections::HashMap<&str, &str>> = std::collections::HashMap::from([
            ("accessToken", std::collections::HashMap::from([("foundry", "access_token")])),
            ("tokenType", std::collections::HashMap::from([("foundry", "token_type")])),
            ("expiresIn", std::collections::HashMap::from([("foundry", "expires_in")])),
            ("refreshToken", std::collections::HashMap::from([("foundry", "refresh_token")])),
            ("scope", std::collections::HashMap::from([("foundry", "scope")])),
        ]);
