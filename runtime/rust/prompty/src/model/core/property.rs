    ...
    pub fn to_value(&self, ctx: &SaveContext) -> serde_json::Value {
        ...
        match &self.kind {
            ...
            PropertyKind::Union { one_of, any_of,  .. } => {
                if !one_of.is_empty() {
                    result.insert("oneOf".to_string(), serde_json::Value::Array(one_of.iter().map(|item| item.to_value(ctx)).collect()));
                }
                if !any_of.is_empty() {
                    result.insert("anyOf".to_string(), serde_json::Value::Array(any_of.iter().map(|item| item.to_value(ctx)).collect()));
                }
            }
