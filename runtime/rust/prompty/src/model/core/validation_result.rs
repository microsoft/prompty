...
impl ValidationResult {
    ...
    pub(crate) fn validate_input_at(value: &serde_json::Value, path: &str) -> Result<(), String> {
        if let Some(entries) = value.get("errors").and_then(|candidate| candidate.as_array()) {
            let collection_path = if path.is_empty() { "errors".to_string() } else { format!("{}.errors", path) };
            for entry in entries {
                ValidationError::validate_input_at(entry, &collection_path)?;
            }
        }
        Ok(())
    }
...
// Serde for `ValidationResult` delegates to the canonical to_value/load_from_value
