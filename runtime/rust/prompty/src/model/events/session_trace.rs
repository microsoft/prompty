...
impl SessionTrace {
    ...
    pub fn load_from_value(value: &serde_json::Value, ctx: &LoadContext) -> Self {
        ...
        Self {
            ...
            turns: value.get("turns").map(|v| Self::load_turns(v, ctx)).unwrap_or_default(),
            checkpoints: value.get("checkpoints").map(|v| Self::load_checkpoints(v, ctx)).unwrap_or_default(),
            trajectory: value.get("trajectory").map(|v| Self::load_trajectory(v, ctx)).unwrap_or_default(),
            files: value.get("files").map(|v| Self::load_files(v, ctx)).unwrap_or_default(),
            refs: value.get("refs").map(|v| Self::load_refs(v, ctx)).unwrap_or_default(),
