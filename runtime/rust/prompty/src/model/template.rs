...
impl Template {
    ...
    pub fn load_from_value(value: &serde_json::Value) -> Self {
        Self {
            format: value.get("format").filter(|v| v.is_object() || v.is_array()).map(Format::load_from_value).unwrap_or_default(),
            parser: value.get("parser").filter(|v| v.is_object() || v.is_array()).map(Parser::load_from_value).unwrap_or_default(),
        }
    }
