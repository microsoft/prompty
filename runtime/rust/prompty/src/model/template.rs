...
impl Template {
    ...
    pub fn load_from_value(value: &serde_json::Value) -> Self {
        Self {
            format: value.get("format").filter(|v| v.is_object() || v.is_array() || v.is_string()).map(FormatConfig::load_from_value).unwrap_or_default(),
            parser: value.get("parser").filter(|v| v.is_object() || v.is_array() || v.is_string()).map(ParserConfig::load_from_value).unwrap_or_default(),
        }
    }
