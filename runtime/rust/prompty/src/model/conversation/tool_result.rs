...
impl<'de> serde::Deserialize<'de> for ToolResultStatus {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str_opt(&s)
            .ok_or_else(|| serde::de::Error::custom(format!("invalid ToolResultStatus value: {}", s)))
    }
}
...
impl ToolResult {
    ...
    pub(crate) fn validate_input_at(value: &serde_json::Value, path: &str) -> Result<(), String> {
        if let Some(entries) = value.get("parts").and_then(|candidate| candidate.as_array()) {
            let collection_path = if path.is_empty() { "parts".to_string() } else { format!("{}.parts", path) };
    ...
    pub fn to_value(&self, ctx: &SaveContext) -> serde_json::Value {
        ...
        if let Some(val) = self.error_message.as_ref() {
