...
use prompty::model::FinalOutputPolicyRequest;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_final_output_policy_request_load_json() {
    let json = r####"
{
  "sessionId": "sample",
  "turnId": "sample",
...
"####;
    ...
    let result = FinalOutputPolicyRequest::from_json(json, &ctx);
    assert!(result.is_ok(), "Failed to load from JSON: {:?}", result.err());
    let instance = result.unwrap();
    assert_eq!(instance.session_id, "sample");
    assert_eq!(instance.turn_id, "sample");
    assert_eq!(instance.iteration, 1);
}

#[test]
fn test_final_output_policy_request_load_yaml() {
    let yaml = r####"
sessionId: sample
turnId: sample
iteration: 1
...
"####;
    let ctx = LoadContext::default();
    let result = FinalOutputPolicyRequest::from_yaml(yaml, &ctx);
...
#[test]
fn test_final_output_policy_request_roundtrip() {
    let json = r####"
{
  "sessionId": "sample",
  "turnId": "sample",
...
"####;
    ...
    let result = FinalOutputPolicyRequest::from_json(json, &load_ctx);
...
#[cfg(feature = "serde")]
#[test]
fn test_final_output_policy_request_serde_roundtrip() {
    let json = r####"
{
  "sessionId": "sample",
  "turnId": "sample",
...
"####;
    let instance: FinalOutputPolicyRequest = serde_json::from_str(json)
    ...
    assert_eq!(instance, FinalOutputPolicyRequest::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    let reparsed: FinalOutputPolicyRequest = serde_json::from_value(value)
