...
use prompty::model::ReplayMismatch;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_replay_mismatch_load_json() {
    let json = r####"
{
  "index": 1,
  "message": "sample"
...
"####;
    ...
    let result = ReplayMismatch::from_json(json, &ctx);
    assert!(result.is_ok(), "Failed to load from JSON: {:?}", result.err());
    let instance = result.unwrap();
    assert_eq!(instance.index, 1);
    assert_eq!(instance.message, "sample");
}

#[test]
fn test_replay_mismatch_load_yaml() {
    let yaml = r####"
index: 1
message: sample
...
"####;
    let ctx = LoadContext::default();
    let result = ReplayMismatch::from_yaml(yaml, &ctx);
...
#[test]
fn test_replay_mismatch_roundtrip() {
    let json = r####"
{
  "index": 1,
  "message": "sample"
...
"####;
    ...
    let result = ReplayMismatch::from_json(json, &load_ctx);
...
#[cfg(feature = "serde")]
#[test]
fn test_replay_mismatch_serde_roundtrip() {
    let json = r####"
{
  "index": 1,
  "message": "sample"
...
"####;
    let instance: ReplayMismatch = serde_json::from_str(json)
    ...
    assert_eq!(instance, ReplayMismatch::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: ReplayMismatch = serde_json::from_value(value)
