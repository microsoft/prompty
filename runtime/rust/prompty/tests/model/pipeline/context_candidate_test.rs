...
use prompty::model::ContextCandidate;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_context_candidate_load_json() {
    let json = r####"
{
...
"####;
    ...
    let result = ContextCandidate::from_json(json, &ctx);
    assert!(result.is_ok(), "Failed to load from JSON: {:?}", result.err());
    let instance = result.unwrap();
    assert_eq!(instance.id, "memory:project-plan");
    assert_eq!(instance.source, "memory");
}

#[test]
fn test_context_candidate_load_yaml() {
    let yaml = r####"
id: "memory:project-plan"
source: memory
...
"####;
    let ctx = LoadContext::default();
    let result = ContextCandidate::from_yaml(yaml, &ctx);
...
#[test]
fn test_context_candidate_roundtrip() {
    let json = r####"
{
  "id": "memory:project-plan",
  "source": "memory"
...
"####;
    ...
    let result = ContextCandidate::from_json(json, &load_ctx);
...
#[test]
fn test_context_candidate_serde_roundtrip() {
    let json = r####"
{
  "id": "memory:project-plan",
  "source": "memory"
...
"####;
    let instance: ContextCandidate = serde_json::from_str(json)
    ...
    assert_eq!(instance, ContextCandidate::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    let reparsed: ContextCandidate = serde_json::from_value(value)
