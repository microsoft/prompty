...
use prompty::model::ModelReconciliationState;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_model_reconciliation_state_load_json() {
    let json = r####"
{
...
"####;
    ...
    let result = ModelReconciliationState::from_json(json, &ctx);
    assert!(result.is_ok(), "Failed to load from JSON: {:?}", result.err());
    let instance = result.unwrap();
    assert_eq!(instance.invocation_id, "inv_abc123");
    assert_eq!(instance.message, "provider connection dropped after request was sent");
}

#[test]
fn test_model_reconciliation_state_load_yaml() {
    let yaml = r####"
invocationId: inv_abc123
message: provider connection dropped after request was sent
...
"####;
    let ctx = LoadContext::default();
    let result = ModelReconciliationState::from_yaml(yaml, &ctx);
...
#[test]
fn test_model_reconciliation_state_roundtrip() {
    let json = r####"
{
  "invocationId": "inv_abc123",
  "message": "provider connection dropped after request was sent"
...
"####;
    ...
    let result = ModelReconciliationState::from_json(json, &load_ctx);
...
#[test]
fn test_model_reconciliation_state_serde_roundtrip() {
    let json = r####"
{
  "invocationId": "inv_abc123",
  "message": "provider connection dropped after request was sent"
...
"####;
    let instance: ModelReconciliationState = serde_json::from_str(json)
    ...
    assert_eq!(instance, ModelReconciliationState::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    let reparsed: ModelReconciliationState = serde_json::from_value(value)
