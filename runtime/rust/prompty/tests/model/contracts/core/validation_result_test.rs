...
use prompty::model::ValidationResult;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_validation_result_load_json() {
    let json = r####"
{
  "valid": true,
  "errors": []
...
"####;
    ...
    let result = ValidationResult::from_json(json, &ctx);
    assert!(result.is_ok(), "Failed to load from JSON: {:?}", result.err());
    let instance = result.unwrap();
    assert_eq!(instance.valid, true);
}

#[test]
fn test_validation_result_load_yaml() {
    let yaml = r####"
valid: true
errors: []
...
"####;
    let ctx = LoadContext::default();
    let result = ValidationResult::from_yaml(yaml, &ctx);
...
"####;
    ...
    let result = ValidationResult::from_json(json, &load_ctx);
...
"####;
    let instance: ValidationResult = serde_json::from_str(json)
    ...
    assert_eq!(instance, ValidationResult::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    let reparsed: ValidationResult = serde_json::from_value(value)
