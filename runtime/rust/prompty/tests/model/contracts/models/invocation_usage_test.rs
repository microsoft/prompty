...
#[test]
fn test_invocation_usage_wire_conversion() {
    let json = r####"
{
  "inputTokens": 150,
  "outputTokens": 42,
...
"####;
    let ctx = LoadContext::default();
    let instance = InvocationUsage::from_json(json, &ctx).expect("load should succeed");
    ...
    assert!(
        !openai_obj.contains_key("inputTokens"),
        "Expected openai wire output to omit inputTokens"
    );
    assert!(
    ...
    assert!(
        !openai_obj.contains_key("outputTokens"),
        "Expected openai wire output to omit outputTokens"
    );
    assert!(
    ...
    let openai_restored = InvocationUsage::from_wire("openai", &openai_wire, &ctx);
    ...
    assert!(
        !anthropic_obj.contains_key("inputTokens"),
        "Expected anthropic wire output to omit inputTokens"
    );
    assert!(
    ...
    assert!(
        !anthropic_obj.contains_key("outputTokens"),
        "Expected anthropic wire output to omit outputTokens"
    );
    let anthropic_restored = InvocationUsage::from_wire("anthropic", &anthropic_wire, &ctx);
...
#[test]
fn test_invocation_usage_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(
        InvocationUsage::from_json("{", &ctx).is_err(),
