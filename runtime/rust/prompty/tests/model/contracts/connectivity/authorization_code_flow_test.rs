...
use prompty::model::AuthorizationCodeFlow;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_authorization_code_flow_load_json() {
    let json = r####"
{
  "authUrl": "sample",
  "codeVerifier": "sample"
...
"####;
    ...
    let result = AuthorizationCodeFlow::from_json(json, &ctx);
    assert!(result.is_ok(), "Failed to load from JSON: {:?}", result.err());
    let instance = result.unwrap();
    assert_eq!(instance.auth_url, "sample");
    assert_eq!(instance.code_verifier, "sample");
}

#[test]
fn test_authorization_code_flow_load_yaml() {
    let yaml = r####"
authUrl: sample
codeVerifier: sample
...
"####;
    let ctx = LoadContext::default();
    let result = AuthorizationCodeFlow::from_yaml(yaml, &ctx);
...
#[test]
fn test_authorization_code_flow_roundtrip() {
    let json = r####"
{
  "authUrl": "sample",
  "codeVerifier": "sample"
...
"####;
    ...
    let result = AuthorizationCodeFlow::from_json(json, &load_ctx);
...
#[cfg(feature = "serde")]
#[test]
fn test_authorization_code_flow_serde_roundtrip() {
    let json = r####"
{
  "authUrl": "sample",
  "codeVerifier": "sample"
...
"####;
    let instance: AuthorizationCodeFlow = serde_json::from_str(json)
    ...
    assert_eq!(instance, AuthorizationCodeFlow::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: AuthorizationCodeFlow = serde_json::from_value(value)
