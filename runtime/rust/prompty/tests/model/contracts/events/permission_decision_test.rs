...
use prompty::model::PermissionDecision;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_permission_decision_load_json() {
    let json = r####"
{
...
"####;
    ...
    let result = PermissionDecision::from_json(json, &ctx);
    assert!(
        result.is_ok(),
...
#[test]
fn test_permission_decision_load_yaml() {
    let yaml = r####"
requestId: perm_abc123
toolCallId: call_abc123
permission: tool.execute
...
"####;
    let ctx = LoadContext::default();
    let result = PermissionDecision::from_yaml(yaml, &ctx);
    assert!(
        result.is_ok(),
...
"####;
    ...
    let result = PermissionDecision::from_json(json, &load_ctx);
...
#[cfg(feature = "serde")]
#[test]
fn test_permission_decision_serde_roundtrip() {
    let json = r####"
{
  "requestId": "perm_abc123",
  "toolCallId": "call_abc123",
...
"####;
    let instance: PermissionDecision =
    ...
    assert_eq!(
        instance,
        PermissionDecision::load_from_value(&canonical, &LoadContext::default()),
    ...
    let reparsed: PermissionDecision =
...
#[test]
fn test_permission_decision_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(
        PermissionDecision::from_json("{", &ctx).is_err(),
