...
use prompty::model::PermissionRequest;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_permission_request_load_json() {
    let json = r####"
{
...
"####;
    ...
    let result = PermissionRequest::from_json(json, &ctx);
    assert!(
        result.is_ok(),
...
#[test]
fn test_permission_request_load_yaml() {
    let yaml = r####"
requestId: perm_abc123
toolCallId: call_abc123
permission: tool.execute
...
"####;
    let ctx = LoadContext::default();
    let result = PermissionRequest::from_yaml(yaml, &ctx);
    assert!(
        result.is_ok(),
...
"####;
    ...
    let result = PermissionRequest::from_json(json, &load_ctx);
...
#[test]
fn test_permission_request_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(
        PermissionRequest::from_json("{", &ctx).is_err(),
