...
"####;
    ...
    assert!(instance.start_time.is_some(), "Expected start_time to be Some");
    assert_eq!(instance.start_time.as_ref().unwrap(), &"2026-06-09T20:00:00Z");
    assert!(instance.selected_model.is_some(), "Expected selected_model to be Some");
    assert_eq!(instance.selected_model.as_ref().unwrap(), &"gpt-4o-mini");
    assert!(instance.reasoning_effort.is_some(), "Expected reasoning_effort to be Some");
...
#[cfg(feature = "serde")]
#[test]
fn test_session_start_payload_serde_roundtrip() {
    let json = r####"
{
  "sessionId": "sess_abc123",
  "schemaVersion": "1",
...
"####;
    let instance: SessionStartPayload = serde_json::from_str(json)
    ...
    assert_eq!(instance, SessionStartPayload::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: SessionStartPayload = serde_json::from_value(value)
