...
use prompty::model::ToolExecutionCompletePayload;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_tool_execution_complete_payload_load_json() {
    let json = r####"
{
...
"####;
    ...
    let result = ToolExecutionCompletePayload::from_json(json, &ctx);
    ...
    assert!(instance.exit_code.is_some(), "Expected exit_code to be Some");
...
#[test]
fn test_tool_execution_complete_payload_load_yaml() {
    let yaml = r####"
requestId: exec_abc123
toolCallId: call_abc123
toolName: powershell
...
"####;
    let ctx = LoadContext::default();
    let result = ToolExecutionCompletePayload::from_yaml(yaml, &ctx);
...
#[test]
fn test_tool_execution_complete_payload_roundtrip() {
    let json = r####"
{
  "requestId": "exec_abc123",
  "toolCallId": "call_abc123",
...
"####;
    ...
    let result = ToolExecutionCompletePayload::from_json(json, &load_ctx);
...
"####;
    let instance: ToolExecutionCompletePayload = serde_json::from_str(json)
    ...
    assert_eq!(instance, ToolExecutionCompletePayload::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    let reparsed: ToolExecutionCompletePayload = serde_json::from_value(value)
