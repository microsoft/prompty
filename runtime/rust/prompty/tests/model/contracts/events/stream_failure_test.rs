...
use prompty::model::StreamFailure;
use prompty::model::StreamFailureOutcome;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_stream_failure_load_json() {
    let json = r####"
{
...
"####;
    ...
    let result = StreamFailure::from_json(json, &ctx);
    assert!(result.is_ok(), "Failed to load from JSON: {:?}", result.err());
    let instance = result.unwrap();
    assert_eq!(instance.outcome, StreamFailureOutcome::Indeterminate);
    assert_eq!(instance.message, "SSE stream error: connection reset");
}

#[test]
fn test_stream_failure_load_yaml() {
    let yaml = r####"
outcome: indeterminate
message: "SSE stream error: connection reset"
...
"####;
    let ctx = LoadContext::default();
    let result = StreamFailure::from_yaml(yaml, &ctx);
...
#[test]
fn test_stream_failure_roundtrip() {
    let json = r####"
{
  "outcome": "indeterminate",
  "message": "SSE stream error: connection reset"
...
"####;
    ...
    let result = StreamFailure::from_json(json, &load_ctx);
...
#[cfg(feature = "serde")]
#[test]
fn test_stream_failure_serde_roundtrip() {
    let json = r####"
{
  "outcome": "indeterminate",
  "message": "SSE stream error: connection reset"
...
"####;
    let instance: StreamFailure = serde_json::from_str(json)
    ...
    assert_eq!(instance, StreamFailure::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: StreamFailure = serde_json::from_value(value)
