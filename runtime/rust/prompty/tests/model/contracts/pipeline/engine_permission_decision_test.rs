...
use prompty::model::EnginePermissionDecision;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_engine_permission_decision_load_json() {
    let json = r####"
{
  "approved": true
}
"####;
    ...
    let result = EnginePermissionDecision::from_json(json, &ctx);
...
#[test]
fn test_engine_permission_decision_load_yaml() {
    let yaml = r####"
approved: true

"####;
    let ctx = LoadContext::default();
    let result = EnginePermissionDecision::from_yaml(yaml, &ctx);
...
#[test]
fn test_engine_permission_decision_roundtrip() {
    let json = r####"
{
  "approved": true
}
"####;
    ...
    let result = EnginePermissionDecision::from_json(json, &load_ctx);
...
#[cfg(feature = "serde")]
#[test]
fn test_engine_permission_decision_serde_roundtrip() {
    let json = r####"
{
  "approved": true
}
"####;
    let instance: EnginePermissionDecision = serde_json::from_str(json)
    ...
    assert_eq!(instance, EnginePermissionDecision::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: EnginePermissionDecision = serde_json::from_value(value)
