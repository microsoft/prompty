...
"####;
    let instance: ModelInvocationRequest =
    ...
    assert_eq!(
        instance,
        ModelInvocationRequest::load_from_value(&canonical, &LoadContext::default()),
        "serde deserialize must equal canonical load_from_value"
    );
    let reparsed: ModelInvocationRequest =
...
#[test]
fn test_model_invocation_request_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(
        ModelInvocationRequest::from_json("{", &ctx).is_err(),
