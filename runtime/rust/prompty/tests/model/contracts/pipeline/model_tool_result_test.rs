...
"####;
    ...
    assert_eq!(
        instance,
        ModelToolResult::load_from_value(&canonical, &LoadContext::default()),
        "serde deserialize must equal canonical load_from_value"
    );
    let reparsed: ModelToolResult =
...
#[test]
fn test_model_tool_result_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(
        ModelToolResult::from_json("{", &ctx).is_err(),
