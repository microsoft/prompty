...
use prompty::model::ContextRequest;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_context_request_load_json() {
    let json = r####"
{
  "sessionId": "sample",
  "turnId": "sample",
...
"####;
    ...
    let result = ContextRequest::from_json(json, &ctx);
    ...
    assert_eq!(instance.invocation_id, "sample");
    assert_eq!(instance.iteration, 1);
}

#[test]
fn test_context_request_load_yaml() {
    let yaml = r####"
sessionId: sample
turnId: sample
invocationId: sample
...
"####;
    let ctx = LoadContext::default();
    let result = ContextRequest::from_yaml(yaml, &ctx);
...
"####;
    ...
    let result = ContextRequest::from_json(json, &load_ctx);
...
"####;
    let instance: ContextRequest = serde_json::from_str(json)
    ...
    assert_eq!(instance, ContextRequest::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    let reparsed: ContextRequest = serde_json::from_value(value)
