...
{}
"####;
    let instance: InvocationContextState =
    ...
    assert_eq!(
        instance,
        InvocationContextState::load_from_value(&canonical, &LoadContext::default()),
        "serde deserialize must equal canonical load_from_value"
    );
    let reparsed: InvocationContextState =
...
#[test]
fn test_invocation_context_state_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(
        InvocationContextState::from_json("{", &ctx).is_err(),
