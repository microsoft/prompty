...
"####;
    ...
    assert_eq!(
        instance,
        ModelReconciliationState::load_from_value(&canonical, &LoadContext::default()),
        "serde deserialize must equal canonical load_from_value"
    );
    let reparsed: ModelReconciliationState =
...
#[test]
fn test_model_reconciliation_state_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(
        ModelReconciliationState::from_json("{", &ctx).is_err(),
