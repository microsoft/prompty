...
use prompty::model::ReplayJournalRecord;
use prompty::model::ReplayRecordKind;
use prompty::model::ReplayRecordStatus;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_replay_journal_record_load_json() {
    let json = r####"
{
  "kind": "session"
}
"####;
    ...
    let result = ReplayJournalRecord::from_json(json, &ctx);
    assert!(result.is_ok(), "Failed to load from JSON: {:?}", result.err());
    let instance = result.unwrap();
    assert_eq!(instance.kind, ReplayRecordKind::Session);
}

#[test]
fn test_replay_journal_record_load_yaml() {
    let yaml = r####"
kind: session

"####;
    let ctx = LoadContext::default();
    let result = ReplayJournalRecord::from_yaml(yaml, &ctx);
...
#[test]
fn test_replay_journal_record_roundtrip() {
    let json = r####"
{
  "kind": "session"
}
"####;
    ...
    let result = ReplayJournalRecord::from_json(json, &load_ctx);
...
#[cfg(feature = "serde")]
#[test]
fn test_replay_journal_record_serde_roundtrip() {
    let json = r####"
{
  "kind": "session"
}
"####;
    let instance: ReplayJournalRecord = serde_json::from_str(json)
    ...
    assert_eq!(instance, ReplayJournalRecord::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: ReplayJournalRecord = serde_json::from_value(value)
