...
use prompty::model::MemoryStore;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_memory_store_load_json() {
    let json = r####"
{
  "entries": []
}
"####;
    ...
    let result = MemoryStore::from_json(json, &ctx);
    assert!(
        result.is_ok(),
...
#[test]
fn test_memory_store_load_yaml() {
    let yaml = r####"
entries: []

"####;
    let ctx = LoadContext::default();
    let result = MemoryStore::from_yaml(yaml, &ctx);
    assert!(
        result.is_ok(),
...
"####;
    ...
    let result = MemoryStore::from_json(json, &load_ctx);
...
#[cfg(feature = "serde")]
#[test]
fn test_memory_store_serde_roundtrip() {
    let json = r####"
{
  "entries": []
}
"####;
    let instance: MemoryStore =
    ...
    assert_eq!(
        instance,
        MemoryStore::load_from_value(&canonical, &LoadContext::default()),
        "serde deserialize must equal canonical load_from_value"
    );
    let reparsed: MemoryStore = serde_json::from_value(value).expect("serde should re-deserialize");
    assert_eq!(instance, reparsed, "serde round-trip must be stable");
}

#[test]
fn test_memory_store_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(
        MemoryStore::from_json("{", &ctx).is_err(),
