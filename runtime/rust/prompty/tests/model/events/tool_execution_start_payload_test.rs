...
"####;
    ...
    assert!(instance.working_directory.is_some(), "Expected working_directory to be Some");
    assert_eq!(instance.working_directory.as_ref().unwrap(), &"/workspace/project");
}
...
"####;
    let instance: ToolExecutionStartPayload = serde_json::from_str(json)
    ...
    assert_eq!(instance, ToolExecutionStartPayload::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: ToolExecutionStartPayload = serde_json::from_value(value)
