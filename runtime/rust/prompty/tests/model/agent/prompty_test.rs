...
"####;
    ...
    assert!(map_value.get("tools").map(|v| v.is_object()).unwrap_or(false), "keyed collection loaded from a MAP must re-serialize to the canonical name-keyed map");
}
...
"####;
    ...
    let map_value = serde_json::to_value(&from_map)
        .expect("serde should serialize the map-loaded instance");
    assert!(map_value.get("outputs").map(|v| v.is_object()).unwrap_or(false), "keyed collection loaded from a MAP must re-serialize to the canonical name-keyed map");
    assert!(map_value.get("tools").map(|v| v.is_object()).unwrap_or(false), "keyed collection loaded from a MAP must re-serialize to the canonical name-keyed map");
}
...
"####;
    ...
    let map_value = serde_json::to_value(&from_map)
        .expect("serde should serialize the map-loaded instance");
    assert!(map_value.get("inputs").map(|v| v.is_object()).unwrap_or(false), "keyed collection loaded from a MAP must re-serialize to the canonical name-keyed map");
    assert!(map_value.get("tools").map(|v| v.is_object()).unwrap_or(false), "keyed collection loaded from a MAP must re-serialize to the canonical name-keyed map");
}
