...
"####;
    ...
    assert!(instance.refresh_token.is_some(), "Expected refresh_token to be Some");
    assert_eq!(instance.refresh_token.as_ref().unwrap(), &"0.AXoAoffline-refresh-token-value");
    assert!(instance.scope.is_some(), "Expected scope to be Some");
    assert_eq!(instance.scope.as_ref().unwrap(), &"https://cognitiveservices.azure.com/.default offline_access");
}
...
"####;
    let instance: OAuthToken = serde_json::from_str(json)
    ...
    assert_eq!(instance, OAuthToken::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    let reparsed: OAuthToken = serde_json::from_value(value)
