...
"####;
    ...
    assert_eq!(instance.description.as_ref().unwrap(), &"Get the current weather for a city");
}
...
#[cfg(feature = "serde")]
#[test]
fn test_anthropic_tool_definition_serde_roundtrip() {
    let json = r####"
{
  "name": "get_weather",
  "description": "Get the current weather for a city"
...
"####;
    let instance: AnthropicToolDefinition = serde_json::from_str(json)
    ...
    assert_eq!(instance, AnthropicToolDefinition::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    let reparsed: AnthropicToolDefinition = serde_json::from_value(value)
