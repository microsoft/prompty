...
use prompty::model::AnthropicImageBlock;
use prompty::model::context::{LoadContext, SaveContext};

#[test]
fn test_anthropic_image_block_load_json() {
    let json = r####"
{
  "type": "image",
  "source": {
...
"####;
    ...
    let result = AnthropicImageBlock::from_json(json, &ctx);
    assert!(result.is_ok(), "Failed to load from JSON: {:?}", result.err());
    let instance = result.unwrap();
    assert_eq!(instance.r#type, "image");
}

#[test]
fn test_anthropic_image_block_load_yaml() {
    let yaml = r####"
type: image
source:
  type: base64
...
"####;
    let ctx = LoadContext::default();
    let result = AnthropicImageBlock::from_yaml(yaml, &ctx);
...
#[test]
fn test_anthropic_image_block_roundtrip() {
    let json = r####"
{
  "type": "image",
  "source": {
...
"####;
    ...
    let result = AnthropicImageBlock::from_json(json, &load_ctx);
...
#[cfg(feature = "serde")]
#[test]
fn test_anthropic_image_block_serde_roundtrip() {
    let json = r####"
{
  "type": "image",
  "source": {
...
"####;
    let instance: AnthropicImageBlock = serde_json::from_str(json)
    ...
    assert_eq!(instance, AnthropicImageBlock::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: AnthropicImageBlock = serde_json::from_value(value)
