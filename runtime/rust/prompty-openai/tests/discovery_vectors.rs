...
use prompty_openai::models::model_info_from_wire;
...
#[test]
fn openai_discovery_vectors_map_to_canonical_model_info() {
    let vectors = load_vectors();
    let mut ran = 0;
    for vector in &vectors {
        if vector["provider"].as_str() != Some("openai") {
            continue;
        }
    ...
    assert!(ran > 0, "no openai discovery vectors were exercised");
}
