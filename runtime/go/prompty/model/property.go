...
func LoadProperty(data interface{}, ctx *LoadContext) (interface{}, error) {
	...
	if m, ok := data.(map[string]interface{}); ok {
		if discriminator, ok := m["kind"]; ok {
			...
			case string:
				if discriminator == "" {
					return nil, fmt.Errorf("invalid Property discriminator field 'kind': expected non-blank string")
				}
			...
			return nil, fmt.Errorf("missing Property discriminator property: kind")
		}
	}
	// Load from map
	if m, ok := data.(map[string]interface{}); ok {
