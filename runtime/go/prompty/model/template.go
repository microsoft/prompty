...
func LoadTemplate(data interface{}, ctx *LoadContext) (Template, error) {
	...
	if m, ok := data.(map[string]interface{}); ok {
		if requiredValue, exists := m["format"]; !exists || requiredValue == nil {
			return result, fmt.Errorf("%s: missing required field", ctx.At("format").Path)
		}
		if requiredValue, exists := m["parser"]; !exists || requiredValue == nil {
			return result, fmt.Errorf("%s: missing required field", ctx.At("parser").Path)
		}
		if val, ok := m["format"]; ok && val != nil {
			if m, ok := val.(map[string]interface{}); ok {
				loaded, err := LoadFormatConfig(m, ctx.At("format"))
				if err != nil {
					return result, err
				}
		...
		if val, ok := m["parser"]; ok && val != nil {
			if m, ok := val.(map[string]interface{}); ok {
				loaded, err := LoadParserConfig(m, ctx.At("parser"))
				if err != nil {
					return result, err
				}
