...
#pragma warning restore CS8618
    ...
    public static MemoryStore Load(Dictionary<string, object?> data, LoadContext? context = null)
    {
        ...
        {
            instance.Entries = LoadEntries(entriesValue, context!.At("entries"));
        }
            ...
            foreach (var kvp in dict)
            {
                if (kvp.Value is IEnumerable<object>)
                {
                    throw new ArgumentException(
                        $"{(string.IsNullOrEmpty(context?.Path) ? "entries" : context.Path)}.{kvp.Key}: invalid named collection entry category array. " +
                ...
                if (itemDict.Count > 0)
                {
                    ...
                    result.Add(MemoryEntry.Load(itemDict, context?.At(kvp.Key)));
                }
                else
                {
                    ...
                    result.Add(MemoryEntry.Load(newDict, context?.At(kvp.Key)));
                }
            }
        }
