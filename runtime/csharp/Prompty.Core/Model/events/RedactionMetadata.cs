...
#pragma warning restore CS8618
    ...
    public static RedactionMetadata Load(Dictionary<string, object?> data, LoadContext? context = null)
    {
        ...
        {
            instance.Fields = LoadFields(fieldsValue, context!.At("fields"));
        }
            ...
            foreach (var kvp in dict)
            {
                if (kvp.Value is IEnumerable<object>)
                {
                    throw new ArgumentException(
                        $"{(string.IsNullOrEmpty(context?.Path) ? "fields" : context.Path)}.{kvp.Key}: invalid named collection entry category array. " +
                ...
                if (itemDict.Count > 0)
                {
                    ...
                    result.Add(RedactedField.Load(itemDict, context?.At(kvp.Key)));
                }
                else
                {
                    ...
                    result.Add(RedactedField.Load(newDict, context?.At(kvp.Key)));
                }
            }
        }
