...
#pragma warning restore CS8618
    ...
    public new static UnionProperty Load(Dictionary<string, object?> data, LoadContext? context = null)
    {
        ...
        {
            instance.OneOf = LoadOneOf(oneOfValue, context!.At("oneOf"));
        }

        if (data.TryGetValue("anyOf", out var anyOfValue) && anyOfValue is not null)
        {
            instance.AnyOf = LoadAnyOf(anyOfValue, context!.At("anyOf"));
        }
            ...
            foreach (var kvp in dict)
            {
                if (kvp.Value is IEnumerable<object>)
                {
                    throw new ArgumentException(
                        $"{(string.IsNullOrEmpty(context?.Path) ? "oneOf" : context.Path)}.{kvp.Key}: invalid named collection entry category array. " +
    ...
    public static IList<Property> LoadAnyOf(object data, LoadContext? context)
    {
        ...
        if (data is System.Collections.IDictionary)
        {
            ...
            foreach (var kvp in dict)
            {
                ...
                {
                    throw new ArgumentException(
                        $"{(string.IsNullOrEmpty(context?.Path) ? "anyOf" : context.Path)}.{kvp.Key}: invalid named collection entry category array. " +
