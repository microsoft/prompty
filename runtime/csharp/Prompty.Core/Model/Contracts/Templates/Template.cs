...
#pragma warning restore CS8618
    ...
    public static Template Load(Dictionary<string, object?> data, LoadContext? context = null)
    {
        ...
        if (!data.TryGetValue("format", out var requiredFormatValue) || requiredFormatValue is null)
        {
            throw new ArgumentException($"{context!.At("format").Path}: missing required field");
        }

        if (!data.TryGetValue("parser", out var requiredParserValue) || requiredParserValue is null)
        {
            throw new ArgumentException($"{context!.At("parser").Path}: missing required field");
        }
        ...
        {
            instance.Format = FormatConfig.Load(formatValue.GetDictionary(FormatConfig.ShorthandProperty), context!.At("format"));
        }

        if (data.TryGetValue("parser", out var parserValue) && parserValue is not null)
        {
            instance.Parser = ParserConfig.Load(parserValue.GetDictionary(ParserConfig.ShorthandProperty), context!.At("parser"));
        }
