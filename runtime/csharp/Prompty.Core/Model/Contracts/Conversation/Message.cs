...
#pragma warning restore CS8618
    ...
    public static Message Load(Dictionary<string, object?> data, LoadContext? context = null)
    {
        ...
        {
            instance.Parts = LoadParts(partsValue, context!.At("parts"));
        }
            ...
            foreach (var kvp in dict)
            {
                if (kvp.Value is IEnumerable<object>)
                {
                    throw new ArgumentException(
                        $"{(string.IsNullOrEmpty(context?.Path) ? "parts" : context.Path)}.{kvp.Key}: invalid named collection entry category array. " +
                ...
                if (itemDict.Count > 0)
                {
                    ...
                    result.Add(ContentPart.Load(itemDict, context?.At(kvp.Key)));
                }
                else
                {
                    ...
                    result.Add(ContentPart.Load(newDict, context?.At(kvp.Key)));
                }
            }
        }
