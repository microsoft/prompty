            ...
            foreach (var kvp in dict)
            {
                if (kvp.Value is IEnumerable<object>)
                {
                    throw new ArgumentException(
                        $"{(string.IsNullOrEmpty(context?.Path) ? "bindings" : context.Path)}.{kvp.Key}: invalid named collection entry category array. " +
                ...
                if (itemDict.Count > 0)
                {
                    ...
                    result.Add(Binding.Load(itemDict, context?.At(kvp.Key)));
                }
                else
                {
                    ...
                    result.Add(Binding.Load(newDict, context?.At(kvp.Key)));
                }
            }
        }
