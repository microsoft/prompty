...
export function emitGoFileContent(
  ...
  if (hasNonProtocol) {
    emitHeader(lines, packageName, group, types.some(type => type.typeName.name === "UnionProperty"));
  } else {
  ...
  // Go has no inheritance, so concrete variants must carry their inherited
  // fields and load/save assignments explicitly.
  const effectiveTypes = types.map(type => flattenInheritedMembers(type, types));
...
/**
 * Materialize inherited fields and serialization assignments for Go structs.
 *
 * Other emitters preserve inheritance in their target language. Go does not,
 * so a child must duplicate the base shape. Child declarations override a base
 * declaration with the same wire name (notably a discriminator such as
 * `kind`).
 */
function flattenInheritedMembers(type: TypeDecl, types: TypeDecl[]): TypeDecl {
  if (!type.base) return type;

  const base = types.find(candidate => candidate.typeName.name === type.base?.name);
  if (!base) return type;

  const flattenedBase = flattenInheritedMembers(base, types);
  return {
    ...type,
    ...
    load: {
      ...
      assignments: mergeByName(
        ...
        assignment => assignment.sourceName,
      ),
    },
    save: {
      ...type.save,
      assignments: mergeByName(
        ...
        assignment => assignment.targetName,
      ),
    },
  };
}

function mergeByName<T>(base: T[], own: T[], getName: (value: T) => string): T[] {
...
function emitHeader(lines: string[], packageName: string, group: string = "", needsFmt: boolean = false): void {
...
function emitUnionCompositionValidation(type: TypeDecl, lines: string[]): void {
  ...
  lines.push("\t\tif (len(result.OneOf) > 0) == (len(result.AnyOf) > 0) {");
  lines.push('\t\t\treturn result, fmt.Errorf("UnionProperty requires exactly one non-empty composition: oneOf XOR anyOf")');
  lines.push("\t\t}");
}
