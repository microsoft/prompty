...
export function emitGoFileContent(
  ...
  if (hasNonProtocol) {
    emitHeader(lines, packageName);
  } else {
    emitProtocolHeader(lines, packageName);
  }
...
function emitHeader(lines: string[], packageName: string): void {
...
function emitProtocolHeader(lines: string[], packageName: string): void {
  lines.push("// Code generated by AgentSchema emitter; DO NOT EDIT.");
  lines.push("");
  lines.push(`package ${packageName}`);
  lines.push("");
...
function emitStruct(
  ...
  for (const field of type.fields) {
    const goType = getGoFieldType(field.category, field.isOptional, polymorphicTypeNames);
...
function emitLoadScalar(
  ...
  if (assign.isOptional) {
    lines.push(`\t\tif val, ok := m["${assign.sourceName}"]; ok && val != nil {`);
    lines.push(`\t\t\tv := val.(${goType})`);
    lines.push(`\t\t\tresult.${fieldName} = &v`);
    lines.push("\t\t}");
  } else {
    lines.push(`\t\tif val, ok := m["${assign.sourceName}"]; ok && val != nil {`);
    lines.push(`\t\t\tresult.${fieldName} = val.(${goType})`);
    lines.push("\t\t}");
  }
}
