...
function cleanupGeneratedCSharpFiles(relDir: string | undefined): void {
  ...
  for (const name of readdirSync(absDir)) {
    ...
    if (stat.isDirectory()) {
      cleanupGeneratedCSharpFiles(absPath);
      continue;
    }
    if (!stat.isFile() || !name.endsWith(".cs")) {
      continue;
    }

    const contents = readFileSync(absPath, "utf-8");
    if (contents.includes("<auto-generated/>") || contents.includes("Code generated by Prompty emitter")) {
      try { unlinkSync(absPath); } catch { /* ignore — file may be locked */ }
    }
  }
}
...
export const generateCsharp = async (context: EmitContext<PromptyEmitterOptions>, node: TypeNode, emitTarget: EmitTarget, options?: GeneratorOptions) => {
  ...
  // Remove stale flat type files from root output dir (they now live in group subdirectories)
