...
<Tabs>
  <TabItem label="Python">
    ...
    stream = run(agent, inputs={"question": "Write a summary"}, raw=True)
  ...
  <TabItem label="TypeScript">
    ```typescript
    import { load, run, process as processResponse } from "@prompty/core";

    const agent = await load("chat.prompty");
    agent.model.options.additionalProperties.stream = true;

    const stream = await run(agent, { question: "Write a summary" }, { raw: true });
