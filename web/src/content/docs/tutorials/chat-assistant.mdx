...
load → prepare → execute → process pipeline, and how thread inputs work.
...
✅ The **invoke()** one-liner and the **load → prepare → execute → process** pipeline  
...
<CardGrid>
  <LinkCard
    ...
    description="Deep dive into how thread inputs work — nonce expansion, data formats, and best practices."
    href="/core-concepts/conversation-history/"
  />
  <LinkCard
