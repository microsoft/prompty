...
For production workloads, use Microsoft Entra ID with `DefaultAzureCredential`
...
<Tabs>
  ...
  <TabItem label="C#">
    ...
    // Entra ID auth is handled automatically by Prompty.Foundry
    // when connection kind is "reference" — uses DefaultAzureCredential
    var result = await Pipeline.InvokeAsync(
        "foundry-chat-aad.prompty",
...
| | API Key (`kind: key`) | Microsoft Entra ID (`kind: reference`) |
|---|---|---|
