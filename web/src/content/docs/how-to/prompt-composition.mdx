...
3. **Execute** — the child prompt runs as a single prompt invocation
...
<Aside type="caution">
  PromptyTool intentionally does **not** start a nested agent loop. If the child
  prompt needs cancellation, compaction, steering, or its own tool loop, call it
  from application code with `turn()` instead of nesting it behind PromptyTool.
</Aside>
