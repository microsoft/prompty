...
<Aside type="note" title="Model discovery support">
  Anthropic prompt execution is covered by shared provider vectors. Model-listing
  helpers are runtime-specific; choose model ids from Anthropic's published model
  catalog when your runtime does not expose a discovery helper.
</Aside>
...
<Tabs>
  ...
  <TabItem label="C#">
    ...
    // Load the prompt
    var agent = PromptyLoader.Load("anthropic-chat.prompty");

    // Render + parse → message list (no LLM call)
    var messages = await Pipeline.PrepareAsync(agent, new() { ["topic"] = "quantum computing" });
