...
<Aside type="note" title="Model discovery support">
  Rust exposes Anthropic model listing today. Python, TypeScript, and C# can run
  Anthropic prompts but do not yet expose an Anthropic model-listing helper; choose
  model ids from Anthropic's published model catalog.
</Aside>
