...
| Error | Cause | Fix |
|---|---|---|
| `ModuleNotFoundError: No module named 'openai'` | Missing optional dependency (Python) | `uv pip install "prompty[openai]"` |
| `ModuleNotFoundError: No module named 'jinja2'` | Missing renderer dependency (Python) | `uv pip install "prompty[jinja2]"` |
| `ModuleNotFoundError: No module named 'chevron'` | Missing Mustache renderer (Python) | `uv pip install "prompty[mustache]"` |
| `ModuleNotFoundError: No module named 'azure.identity'` | Missing Foundry/Azure dependency (Python) | `uv pip install "prompty[foundry]"` |
...
<Aside type="tip">
**Python:** Install everything at once with `uv pip install "prompty[all]"`.\
**TypeScript:** `npm install @prompty/core @prompty/openai`.\
**C#:** `dotnet add package Prompty.OpenAI` (includes core as a transitive dependency).
</Aside>
...
console.log(resolve("chat.prompty"));  // see what Prompty will look for
```
</TabItem>
<TabItem label="C#">
```csharp
...
<Tabs>
  ...
  <TabItem label="C#">
    ...
    **Fix:** Add the provider NuGet package and register it at startup:
...
const getWeather = tool(
  ...
  "get_weather",
  { description: "Get the weather", parameters: { city: { kind: "string" } } }
);
```
</TabItem>
<TabItem label="C#">
...
[Tool("get_weather", Description = "Get the weather")]
...
var messages = await Pipeline.PrepareAsync(agent, new() { ["question"] = "Hello" });
var result = Pipeline.RunStreamingAsync(agent, messages);
...
<TabItem label="TypeScript">
```typescript
import { Tracer, consoleTracer, invoke } from "@prompty/core";

Tracer.add("console", consoleTracer);
const result = await invoke("chat.prompty", { question: "Hello" });
```
</TabItem>
<TabItem label="C#">
```csharp
...
var agent = PromptyLoader.Load("chat.prompty");
var messages = await Pipeline.PrepareAsync(agent, new() { ["name"] = "Jane" });
