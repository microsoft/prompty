...
Switch to Azure by changing the provider and connection:

```yaml title="embed-azure.prompty"
---
name: azure-embedder
model:
...
```python
vector = execute("embed-azure.prompty", inputs={"text": "Hello from Azure"})
```
...
| `text-embedding-ada-002` | 1536 | OpenAI / Azure |
