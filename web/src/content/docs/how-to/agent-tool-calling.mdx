...
<Tabs>
  ...
  <TabItem label="TypeScript">
    ...
    const result = await invokeAgent("agent.prompty", {
  ...
  <TabItem label="C#">
    ...
    // One-shot invocation with agent loop
    ...
    var messages = await Pipeline.PrepareAsync(
        agent,
        new() { ["question"] = "Weather in NYC?" }
    );
    var stepResult = await Pipeline.InvokeAgentAsync(
        agent,
        new() { ["question"] = "Weather in NYC?" },
        tools: tools
    );
...
<Tabs>
  <TabItem label="Python">
    ...
    async def main():
        result = await prompty.invoke_agent_async(
            "agent.prompty",
  ...
  <TabItem label="C#">
    ...
    var tools = new Dictionary<string, Func<string, Task<string>>>
    {
        ["get_weather"] = async (args) =>
        {
            ...
            var resp = await client.GetStringAsync($"https://api.weather.com/v1/{city}");
            return resp;
        }
    };
    ...
    <Aside type="tip">
      In C#, all tool functions are inherently async (`Func&lt;string, Task&lt;string&gt;&gt;`),
      so there's no separate sync/async distinction.
    </Aside>
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem label="TypeScript">
    ...
    const result = await invokeAgent("research-agent.prompty", {
      inputs: { request: "Find docs about onboarding and email a summary to jane@example.com" },
      tools,
    });
    ```
  ...
  <TabItem label="C#">
    ...
    var tools = new Dictionary<string, Func<string, Task<string>>>
    {
        ...
        {
            var parsed = ToolDispatch.ParseArguments(args);
            var query = parsed["query"]?.ToString() ?? "";
            var limit = parsed.ContainsKey("limit") ? parsed["limit"]?.ToString() : "5";
            return $"Found {limit} results for '{query}'";
        },
        ["get_user"] = async (args) =>
        {
            var parsed = ToolDispatch.ParseArguments(args);
            var email = parsed["email"]?.ToString() ?? "";
            return $$"""{"name": "Jane Doe", "email": "{{email}}", "role": "Engineer"}""";
        },
        ["send_email"] = async (args) =>
        {
            var parsed = ToolDispatch.ParseArguments(args);
            var to = parsed["to"]?.ToString() ?? "";
  ...
  <TabItem label="C#">
    ...
    try
    {
        var result = await Pipeline.InvokeAgentAsync(
            ...
            tools: new Dictionary<string, Func<string, Task<string>>>
            {
                ["get_weather"] = async (args) =>
                {
                    var parsed = ToolDispatch.ParseArguments(args);
                ...
                // Missing get_time — will throw if the LLM calls it!
            }
        );
    }
