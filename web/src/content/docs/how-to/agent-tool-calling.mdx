...
<Aside type="note">
  `apiType: agent` tells the runtime to enable the agent loop. Without it, the
  LLM may return `tool_calls` but the runtime won't execute them.
</Aside>
...
<Tabs>
  <TabItem label="Python">
    ...
    # Execute with tool_functions in metadata
    ...
    **Step-by-step variant:**
    ...
    # Attach tool functions to the agent's metadata
  ...
  <TabItem label="TypeScript">
    ...
    const result = await execute("agent.prompty", {
...
<Tabs>
  ...
  <TabItem label="TypeScript">
    ...
    const result = await execute("agent.prompty", {
      inputs: { question: "Weather in London?" },
      configuration: {
        toolFunctions: { get_weather: getWeather },
      },
    });
    ```
  </TabItem>
...
<Tabs>
  ...
  <TabItem label="TypeScript">
    ...
    const result = await execute("research-agent.prompty", {
      inputs: { request: "Find docs about onboarding and email a summary to jane@example.com" },
      configuration: { toolFunctions },
    });
    ```
  </TabItem>
</Tabs>
...
<Aside type="tip">
  For Azure OpenAI, just change `provider: openai` to `provider: azure` and
  update the connection — the agent loop works identically with both providers.
</Aside>
