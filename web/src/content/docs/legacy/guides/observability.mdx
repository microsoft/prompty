...
def custom_tracer(name: str) -> Iterator[Callable[[str, Any], None]]:
    ...
    try:
        ...
        print(f"📊 {name} completed with {len(traces)} traces")
...
- Learn about [debugging Prompty](/getting-started/debugging-prompty/) for troubleshooting
- Explore [CLI Usage](/guides/cli/) for command-line tracing options
- Check out [Advanced Configuration](/guides/configuration/) for complex setups
