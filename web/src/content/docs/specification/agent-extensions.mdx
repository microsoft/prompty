...
base loop and MAY implement any combination of these extensions. Requirements
inside an extension section apply to implementations that choose to support
that extension.
...
An implementation that supports agent events MUST accept an optional event
callback that receives structured events during execution. This enables
real-time UIs, logging, and coordination without coupling the loop to any
...
| `compaction_start`  | `{}`                                             | Context compaction is about to start      |
| `compaction_complete` | `{ removed: number, remaining: number }`       | Context compaction completed              |
| `compaction_failed` | `{ message: string }`                            | Context compaction failed                 |

### API

```
function turn(path_or_agent, inputs, tools=null,
...
An implementation that supports cancellation MUST use cooperative cancellation
via a token checked at well-defined points during execution.
...
context window. An implementation that supports context window management MUST
support automatic context trimming when a budget is specified.
...
function trim_to_context_window(messages, budget_chars) → Message[]:
  ...
  7. Insert summary as user message immediately after system_messages:
     summary_msg = Message(role: "user",
       parts: [TextPart(value: "[Context summary: " + summary + "]")])
...
The `bind_tools` function validates that `@tool`-decorated handler functions match
the tool declarations in an agent's frontmatter, and returns a handler dictionary
suitable for passing to `turn`.
...
- `bind_tools` MUST only validate against `kind: "function"` tools. Tools with
  other kinds (mcp, openapi, custom) are resolved by kind handlers and do not
