...
The TypeSpec model is the authoritative schema source. Earlier narrative
specification text is retained here only when it describes behavior; type
members, discriminators, inheritance, and property names should be checked
against `schema/` or the generated [Schema Reference](/reference/).
...
:::note[Naming Note]
The pipeline stage that calls the LLM is called **execute** (the `Executor`
protocol). The top-level convenience functions that run the entire pipeline
