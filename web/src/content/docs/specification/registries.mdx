...
- Implementations SHOULD register a built-in kind handler for `"function"`
  at startup. Handlers for `"mcp"`, `"openapi"`, and `"*"`
...
Every component MUST conform to its protocol. The schema defines one logical
contract for each protocol method; runtimes map those methods to idiomatic
sync and async shapes as described in [§12.1](/specification/cross-cutting/).

**Renderer:**

```
Renderer:
  render(agent: PromptAgent, template: string, inputs: Record<unknown>) → string
```

Receives the agent, template text, and user inputs. Returns the rendered
template string with all variables substituted.

**Parser:**

```
Parser:
  preRender(template: string) → unknown?  // optional sync hook
  parse(agent: PromptAgent, rendered: string, context: Record<unknown>?) → Message[]
```
...
Executor:
  ...
  executeStream(agent: PromptAgent, messages: Message[]) → raw_stream  // optional
  formatToolMessages(rawResponse, toolCalls, toolResults, textContent?) → Message[]  // sync
```
...
```
Processor:
  process(agent: PromptAgent, response: unknown) → result
  processStream(stream: unknown) → stream_result  // optional
```
