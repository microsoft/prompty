...
function message_to_wire(message) → dict:
  ...
  if message.parts has exactly 1 element AND that element is a TextPart:
    wire.content = message.parts[0].value        // plain string
  else:
    wire.content = [part_to_wire(part) for part in message.parts]
...
```
function output_schema_to_wire(outputs: Properties) → dict | null:
  if outputs is empty:
    return null
  ...
  for msg in messages:
    ...
    if msg.metadata and "responses_function_call" in msg.metadata:
      ...
      input_items.append({
        ...
        output: msg.parts[0].value if msg.parts else ""
      })
      continue

    // Normal message
    if len(msg.parts) == 1 and msg.parts[0].kind == "text":
      item.content = msg.parts[0].value
    else:
      item.content = [part_to_wire(part) for part in msg.parts]
