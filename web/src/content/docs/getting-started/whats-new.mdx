...
and a cleaner file format backed by the [Prompty schema](/reference/).
...
- Consistent public API across languages:
  ...
  - `turn()` — agentic tool-calling loop

## New File Format

- YAML frontmatter follows the [Prompty schema](/reference/) (defined in TypeSpec)
...
- `turn()` runs a **tool-calling loop** automatically — send a prompt, receive
