...
All packages are available on NuGet at version **2.0.0**.
...
Use `TurnAsync` to run the agent loop — the runtime calls the LLM,
...
- **Resilient JSON parsing** — `ParseArguments()` recovers from malformed tool arguments (markdown fences, trailing commas)
...
<Aside type="tip">
  `TurnAsync` runs the full agent loop — it calls the LLM, executes any
