...
| Azure OpenAI | `azure` | `openai` + `azure-identity` | `prompty[azure]` |

Providers are discovered via Python entry points. Third-party providers
can register themselves in their `pyproject.toml`.
