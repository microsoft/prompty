...
<Tabs>
  <TabItem label="Python">
    ...
    <Aside type="note">
      `invoke_agent()` and `invoke_agent_async()` still work as deprecated aliases for `turn()` and `turn_async()`.
    </Aside>
  </TabItem>
  <TabItem label="TypeScript">
...
<Aside type="caution" title="Deprecated Aliases">
  `invoke_agent()` / `invoke_agent_async()` (Python), `invokeAgent()` (TypeScript), and `Pipeline.InvokeAgentAsync()` (C#) still work but are deprecated. Use `turn()` / `turn_async()`, `turn()`, and `Pipeline.TurnAsync()` respectively.
</Aside>
