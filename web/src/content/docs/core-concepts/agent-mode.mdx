...
Define one or more Python functions, then pass them to `invoke_agent()` along
...
var tools = new Dictionary<string, Func<string, Task<string>>>
{
    ...
    {
        var parsed = ToolDispatch.ParseArguments(args);
        var tz = parsed["timezone"]?.ToString() ?? "UTC";
        return $"3:42 PM in {tz}";
    }
};
...
<Aside type="note">
  ...
  use `invoke_agent()` (Python) or `invokeAgent()` (TypeScript) to enable the
  automatic tool-calling loop. If you use `invoke()` instead, tools are sent to
...
For async applications, use `invoke_agent_async()`. Your tool functions can be
...
<Aside type="caution">
  In **sync** mode (`invoke_agent`), passing an async function raises a
  `ValueError`. Use `invoke_agent_async` when any of your tool functions are
  coroutines.
</Aside>
...
```python
try:
    result = prompty.invoke_agent(agent, inputs, tools, max_iterations=5)
