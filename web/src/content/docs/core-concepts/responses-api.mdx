...
The Responses API is transparent to your calling code. The same `invoke()` and
...
Tool calling works with the Responses API through the same `invoke_agent()` /
`invokeAgent()` functions used for Chat Completions. The agent loop
...
When `outputs` is defined, the runtime converts it to the Responses API's
...
<Aside type="tip">
  ...
  single line in your `.prompty` file. Your calling code (`invoke()`,
  `invoke_agent()`, etc.) stays exactly the same — the runtime handles the wire
