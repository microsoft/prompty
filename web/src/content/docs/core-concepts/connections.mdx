...
Prompty ships six connection types — each backed by a corresponding schema
type — so you can match the auth strategy to your environment.
...
pattern for **production** because it supports Microsoft Entra ID, managed identity,
...
<Tabs>
  ...
  <TabItem label="C#">
    ...
    ConnectionRegistry.Register("my-foundry-client", new AzureOpenAI(
        new Uri("https://my-resource.services.ai.azure.com/api/projects/my-project"),
...
<Tabs>
  <TabItem label="Python">
    ...
    # Build a client with Entra ID token-based auth
    client = AzureOpenAI(
...
<Tabs>
  ...
  <TabItem label="Production (Entra ID)">
...
| Production with Microsoft Entra ID | `reference`     | Full SDK control, managed identity        |
