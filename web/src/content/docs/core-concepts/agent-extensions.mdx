...
```
```python
# ✅ With @tool — typed parameters, no JSON parsing
from prompty import tool, invoke_agent
...
# Pass decorated functions as handlers — names must match the
# tool declarations in your .prompty frontmatter.
result = invoke_agent(agent, inputs, tools={"get_weather": get_weather})
```
</TabItem>
<TabItem label="TypeScript">
```typescript
...
```
```typescript
// ✅ With tool() — typed, structured handler
import { tool, invokeAgent } from "@prompty/core";
...
// DiscoverTools builds typed handlers from [Tool] methods.
// Pass the resulting dict to invoke_agent as tool handlers.
...
<TabItem label="Python">
```python
from prompty import load, invoke_agent, tool
...
<TabItem label="TypeScript">
```typescript
import { load, invokeAgent, tool } from "@prompty/core";
...
const result = await invokeAgent(agent, { question: "Weather in Tokyo?" }, {
  tools: { get_weather: getWeather, get_time: getTime },
});
console.log(result);
```
</TabItem>
...
<Aside type="note">
  Tool **names must match** between the `.prompty` frontmatter and your handler
  dictionary. The frontmatter is the declaration the LLM sees; the handler dict
  is the implementation the runtime calls.
</Aside>
