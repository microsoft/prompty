...
description: >
  ...
  tracing integration, processor handling, and agent-loop compatibility.
sidebar:
  order: 6
---
...
<Tabs>
  ...
  <TabItem label="TypeScript">
    ...
    // processResponse yields text chunks
    for await (const chunk of await processResponse(agent, stream)) {
  ...
  <TabItem label="C#">
    ...
    // RunAsync with raw: true returns the PromptyStream
    ...
    // Or via the processor — returns ProcessedStream (IAsyncEnumerable<string>)
    var processed = await Pipeline.RunAsync(agent, messages);
    if (processed is ProcessedStream ps)
    {
  ...
  <TabItem label="Rust">
    ...
    // Rust run() consumes a streaming provider response and returns
    // the accumulated text result.
...
When using `turn()`, the runtime runs a tool-calling loop:
