...
| `mode` | `single` (default) — one LLM call via `prepare` + `run`. `agentic` — full agent loop via `turn` |
...
`turn()` — which runs the agent loop: call the LLM, execute any
...
<Tabs>
  ...
  <TabItem label="TypeScript">
    ...
    const result = await turn(agent, { question: "Weather in Seattle?" }, { tools });
    ```
  </TabItem>
  <TabItem label="C#">
    ```csharp
...
<Aside type="tip">
  `turn` / `turn` / `TurnAsync` runs the full agent loop —
  it calls the LLM, executes any returned tool calls, appends the results, and
  loops until the model returns a final response. Use `invoke` for
...
<Tabs>
  ...
  <TabItem label="C#">
    ...
    // 1. userTools dict (passed to TurnAsync)
...
| Override wire format for built-in kind | ✅ Yes | No — use `tools` dict in `turn` |
...
<Aside type="tip">
  ...
  functions in the `tools` dict to `turn()`. The registry is for
