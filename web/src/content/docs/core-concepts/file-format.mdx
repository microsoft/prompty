...
flowchart TD
    ...
    Identity -- maps to --> PA1["Prompty.name\n.description\n.metadata"]
    ModelCfg -- maps to --> PA2["Prompty.model"]
    Inputs -- maps to --> PA3["Prompty.inputs"]
    Tools -- maps to --> PA4["Prompty.tools"]
    Template -- maps to --> PA5["Prompty.template"]
    Body -- maps to --> PA6["Prompty.instructions"]
...
<Aside type="caution" title="template shorthand is deprecated">
  The `template` property accepts a string shorthand for backward compatibility,
  but it emits a `DeprecationWarning` and will be removed in a future version.
  Prefer the structured object form:
