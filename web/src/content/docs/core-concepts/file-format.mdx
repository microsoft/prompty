...
**Shorthand (recommended)** — just name the template engine. The parser
defaults to `prompty`:

```yaml
template: jinja2    # expands to {format: {kind: "jinja2"}, parser: {kind: "prompty"}}
```

```yaml
template: mustache  # expands to {format: {kind: "mustache"}, parser: {kind: "prompty"}}
```

**Full form** — use when you need to set the format and parser
independently:
...
String shorthands also work inside the full form — `format: jinja2` is
equivalent to `format: { kind: jinja2 }`, and `parser: prompty` is
equivalent to `parser: { kind: prompty }`:
