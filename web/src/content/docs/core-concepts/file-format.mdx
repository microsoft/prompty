...
[`Agent`](/reference/agent/) type. Here is a summary — see the
...
<Aside type="caution" title="File reference scope">
  `${file:...}` references are scoped to the containing `.prompty` file's
  directory tree by default. Runtimes reject absolute paths, `..` traversal,
  and symlink escapes that resolve outside that tree. Host applications can
  explicitly pass additional allowed roots through load options; a prompt file
  cannot grant itself broader filesystem access.
</Aside>
