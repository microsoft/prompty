...
graph LR
  ...
  subgraph invoke ["invoke() — full pipeline: load → render → parse → execute → process → result"]
...
| `prompty.executors` | `agent.model.provider` | `openai`, `foundry`, `azure`, `anthropic` |
| `prompty.processors` | `agent.model.provider` | `openai`, `foundry`, `azure`, `anthropic` |
...
azure = "prompty.providers.foundry.executor:FoundryExecutor"  # backward-compat alias
...
azure = "prompty.providers.foundry.processor:FoundryProcessor"  # backward-compat alias
