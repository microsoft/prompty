...
1. **Install the NuGet package**: You need to install the `Microsoft.SemanticKernel.Prompty` package. It can be found on NuGet at the following link: [Microsoft.SemanticKernel.Prompty (Alpha)](https://www.nuget.org/packages/Microsoft.SemanticKernel.Prompty/1.24.1-alpha)

   ```
   dotnet add package Microsoft.SemanticKernel.Prompty --version 1.24.1-alpha
   ```

2. **Setup your Semantic Kernel**: Make sure you have the Semantic Kernel ready in your project by adding the necessary dependencies and configuring the kernel.
