...
- Use the _prompty_ command-line tool to execute a `.prompty` asset and "chat" with your model.
...
Prompty supports multiple AI model providers to give you flexibility in choosing the right model for your use case:

### Enterprise, Cloud and ISV Providers
- **Azure OpenAI** - Enterprise-grade OpenAI models with data residency and compliance
- **OpenAI** - Direct integration with OpenAI's latest models
- **Snowflake Cortex** - Fully managed LLM functions within your Snowflake environment
- **Azure Model-as-a-Service (MaaS)** - Serverless models from various providers

### Model Selection Considerations
- **Azure OpenAI**: Best for enterprise applications requiring data governance and compliance
- **OpenAI**: Ideal for development and applications needing the latest model capabilities  
- **Snowflake Cortex**: Ideal for executing within the Snowflake environment with the Data in Snowflake
- **Azure MaaS**: Great for experimenting with diverse models in a serverless environment

Each provider offers different models, capabilities, and pricing structures. See the [Prompty Specification](/docs/prompty-specification) for detailed configuration options.
