...
- Password: `password`
- Key-pair: `private_key` or `private_key_path`
- SSO: `authenticator: externalbrowser`
- OAuth: `authenticator: oauth` with `token`

**Optional Properties:**
- `schema` - Schema to use (default user schema will be assumed)
- `role` - Snowflake role to assume (default use role will be assumed)
- `model` - Cortex model name (default: "llama3.1-8b")

**Supported Models:**
- List of models supported by [Snowflake Cortex](https://docs.snowflake.com/en/user-guide/snowflake-cortex/aisql#availability)
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>object</td>
      <td>Content safety guardrails configuration (Snowflake Cortex)</td>
    </tr>
  </tbody>
</table>
...
### Snowflake Cortex Guardrails

For Snowflake Cortex models, you can configure content safety guardrails:

```yaml
parameters:
  guardrails: true    # Enable content safety guardrails
```

The guardrails parameter can be set to:
- `true` - Enable content safety guardrails
- `false` - Disable content safety guardrails (default)
